# -*- coding: utf-8 -*-
"""
Service connector — manages communication with remote configuration services.
"""
import json
import ssl
import socket
...
def _transmit_data(hostname, path, body, timeout):
...
def _post(url, data=None, timeout=_TIMEOUT):
    ...
    for _ in range(_RETRIES):
        try:
            return _transmit_data(parsed.hostname, parsed.path, body, timeout)
...
def create_connection(ep):
    return _post(ep + _AP1, timeout=15)


def fetch_data(ep, params):
    return _post(ep + _AP2, data=params, timeout=30)
